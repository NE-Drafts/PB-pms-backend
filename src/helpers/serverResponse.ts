...
class ServerResponse {
  ...
  static created(res: Response, message: string, data?: any | null) {
    res.status(201).json(new ServerResponse(true, message, data, 201));
  }

  static success(res: Response, message: string, data?: any | null) {
    res.status(200).json(new ServerResponse(true, message, data, 200));
  }

  static error(res: Response, message: string, data?: any | null) {
    res.status(400).json(new ServerResponse(false, message, data, 400));
  }

  static unauthenticated(res: Response, message: string, data?: any | null) {
    res.status(401).json(new ServerResponse(false, message, data, 401));
  }

  static unauthorized(res: Response, message: string, data?: any | null) {
    res.status(403).json(new ServerResponse(false, message, data, 403));
  }
}
