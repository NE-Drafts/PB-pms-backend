...
const getUserParkingSessions = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const serializedData = convertBigIntToString(parkingSessions);
    return ServerResponse.success(res, "User parking sessions", {
      parkingSessions: serializedData,
    });
  } catch (error) {
    return ServerResponse.error(res, "Internal server error 500");
  }
...
const exitParkingSession = async (req: Request, res: Response) => {
  try {
    ...
    const parkingSession = await prisma.parkingSession.findFirst({
      where: {
        AND: [{ vehicleId: vehicle.id }, { status: "ACTIVE" }],
      },
    });
    ...
    if (!parkingSession)
      return ServerResponse.error(
        res,
        "This vehicle is not in a parking session"
      );

    const exitTime = new Date();
    const hoursSpent = Math.ceil(
      (exitTime.getTime() - parkingSession.entryTime.getTime()) /
        (1000 * 60 * 60)
    );
    const amount = BigInt(
      hoursSpent *
        parseInt((process.env.PARKING_HOURLY_PAY_RATE ?? "5000") as string)
    );
    ...
    return ServerResponse.success(res, "Parking session exited successfully", {
